...
# バックエンド
# 方法1: セットアップスクリプト（環境自動検出、venv作成、依存関係インストール）
python setup.py
...
OpenAI互換の任意のAPI（DeepSeek、Qwen、OpenAI、Ollamaなど）で動作します。
...
OpenClaw Gateway（ポート18789）と連携し、自然言語でAIコーディングアシスタントにローカルタスクを委託します。
...
NagaAgentは4つの独立したマイクロサービスで構成され、すべて`main.py`によってオーケストレーションされます：
...
| OpenClaw Gateway | 18789 | AIコンピュータ制御（オプション） |
...
python update.py                          # 自動git pull + 依存関係同期
```
